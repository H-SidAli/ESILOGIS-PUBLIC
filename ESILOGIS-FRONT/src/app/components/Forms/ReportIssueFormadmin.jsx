...
export default function ReportIssueFormTechnician({ 
  ...
  const processedBarcodeRef = useRef(''); // Ref to track processed barcode to prevent infinite loops
  
  // Update fileList when attachedFiles prop changes
  useEffect(() => {
    if (JSON.stringify(fileList) !== JSON.stringify(attachedFiles)) {
  ...
  useEffect(() => {
    ...
    if (matchingEquipment) {
      // Found a match - batch state updates
      ...
      // No match found - batch state updates
  ...
  // Only add stable dependencies - don't include toast or setBarcodeNotFound
  }, [scannedBarcode, equipmentList, locationId]);

  // Fetch locations - FIXED
  useEffect(() => {
    async function fetchLocations() {
      if (isLoading === false) return; // Prevent duplicate fetches
  ...
  useEffect(() => {
    ...
    async function fetchEquipmentByLocation() {
      try {
        ...
        // Reset equipment state only once we're sure we're fetching new data
        setEquipment("");
        setEquipmentId("");
        setEquipmentSearch("");
        setBarcodeNotFound({ show: false, barcode: '', format: '' });
  ...
  // Fetch priorities once - FIXED
  useEffect(() => {
    async function fetchPriorities() {
      // Only fetch if we don't already have custom priorities
  ...
  const handleLocationSelect = (location) => {
    ...
    // Reset barcode processing state when location changes
    processedBarcodeRef.current = '';
  };
  ...
  return (
    <div className="bg-white px-4 sm:px-6 md:px-8 lg:px-40 py-6 sm:py-8 md:py-10 shadow-xl rounded-lg w-full max-w-7xl mx-auto font-light font-outfit">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        {/* Location Field - Updated to match admin styling */}
        <div>
