...
export default function DashboardContent() {
  ...
  const fetchDashboardData = async () => {
    try {
          ...
          if (process.env.NODE_ENV === 'development') {
            equipmentData = {
              data: [
                ...
                {id: 2, status: 'IN_SERVICE'},
                {id: 3, status: 'UNDER_MAINTENANCE'},
                {id: 4, status: 'OUT_OF_SERVICE'}
              ]
            };
          }
        }
      ...
      // Try to fetch technicians - only if ADMIN role
      try {
        const currentUserRole = JSON.parse(sessionStorage.getItem('user') || '{}').role;
        ...
        if (currentUserRole === 'ADMIN') {
          ...
          // For non-admin users, just use a count of 1 (representing themselves)
          techniciansData = { data: [{ id: 'self' }] };
        }
      } catch (err) {
        loadStatus.technicians = false;
        ...
        // Demo data
        if (process.env.NODE_ENV === 'development') {
          techniciansData = { data: [{ id: 1 }, { id: 2 }] };
        }
      }
      ...
      // Set default data for charts in case of error
      setStats({
        reports: 0,
        completed: 0,
        inProgress: 0,
