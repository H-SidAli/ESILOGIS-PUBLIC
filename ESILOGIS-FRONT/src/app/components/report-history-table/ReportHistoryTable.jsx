...
export default function ReportHistoryTable() {
  ...
  useEffect(() => {
    async function fetchInterventions() {
      setLoading(true);
      try {
        const response = await fetch("http://localhost:3001/api/intervention/my-reported", {
          method: "GET",
          headers: {
  ...
  const handleSubmitFeedback = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/intervention/${currentFeedback.id}/feedback`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${sessionStorage.getItem("token")}`,
        ...
        body: JSON.stringify({
          feedback: currentFeedback.content,
        }),
      });
      ...
      const updatedData = data.map(item => {
        if (item.id === currentFeedback.id) {
          return {
            ...item,
            feedback: 'Your Feedback', // Change button text
            feedbackContent: currentFeedback.content,
            status: 'Completed',
            feedbackType: 'user' // Add this to track that this is user feedback now
          };
        }
        return item;
      });
      setData(updatedData);
      setFilteredData(updatedData.filter(item => filteredData.some(filtered => filtered.id === item.id)));
      setShowFeedbackPopup(false);
    } catch (error) {
      console.error('Error submitting feedback:', error);
      // Even if API fails, optimistically update UI
      const updatedData = data.map(item => {
        if (item.id === currentFeedback.id) {
          return {
            ...item,
        ...
        cell: row => (
          <button
            ...
            className={`text-sm px-3 py-1 rounded-[7px] ${
              row.feedback === 'Your Feedback' || row.feedbackType === 'user'
                ? 'bg-[#EA8B00] text-white'
                : 'bg-[#0060B4] text-white'
            }`}
          >
