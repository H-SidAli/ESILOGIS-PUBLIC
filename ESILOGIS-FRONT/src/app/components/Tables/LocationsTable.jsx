...
import { Search, MoreVertical, Loader, RefreshCw } from 'lucide-react';
...
export default function LocationsTable({ 
  ...
  onDeleteLocation, 
  onEditLocation,
  onRefresh // New prop to trigger data refresh from parent
}) {
  ...
  const [isLoading, setIsLoading] = useState({ delete: false, edit: false, refresh: false });
  ...
  useEffect(() => {
    ...
    setFilteredData(
      locations
        .filter(location => location && typeof location === 'object') // Ensure location is valid
        .filter(location => 
          location.name && // Check that name exists
          typeof location.name === 'string' && // Make sure name is a string
  ...
  const handleSaveEdit = async (id) => {
    ...
    try {
      await onEditLocation(id, editValue);
      setEditMode(null);
  ...
  const DropdownMenu = ({ row }) => createPortal(
    ...
    >
      <button 
        ...
        disabled={isLoading.edit || isLoading.delete || isLoading.refresh}
      >
        Edit
      </button>
      <hr />
      <button 
        ...
        disabled={isLoading.delete || isLoading.refresh}
      >
        {isLoading.delete ? (
          <>
    ...
    <div className="w-full relative font-outfit">
      ...
      <div className="md:hidden border-t border-b border-gray-300 px-4 py-3">
        <div className="flex items-center space-x-2 py-3">
          ...
          <button 
            onClick={refreshData} 
            className="p-2 text-[#0060B4] hover:bg-blue-50 rounded-full"
            disabled={isLoading.refresh}
            title="Refresh data"
            aria-label="Refresh data"
          >
            <RefreshCw size={18} className={isLoading.refresh ? "animate-spin" : ""} />
          </button>
          <button
            onClick={onAddLocation}
      ...
      <div className="table-container w-full relative overflow-x-auto">
        <DataTable
          ...
          progressPending={isLoading.refresh}
          progressComponent={
      ...
      {filteredData.length === 0 && !search && !isLoading.refresh && (
