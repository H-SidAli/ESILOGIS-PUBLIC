  ...
  const handleStatusChange = async (newStatus) => {
    try {
      ...
      const updatePromises = selectedRows.map(async (taskId) => {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to update task ${taskId}: ${response.status}`
          );
        }
      ...
      setData((prevData) =>
        prevData.map((task) =>
          selectedRows.includes(task.id) ? { ...task, status: newStatus } : task
        )
      );
...
const router =useRouter();
  const handleview = () => {router.push("/admin/ReportDetails")};
  const handleedit = () => {router.push("/admin/ReportDetailsEdit")};
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch("https://run.mocky.io/v3/204e0017-a896-4fa7-b489-10cebe660d3b", {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
        ...
        const formattedData = result.map((task) => {
          ...
          return {
            ...
            status: mapStatusFromBackend(task.status), // Use the mapping function
            priority: mapPriorityFromBackend(task.priority), // Use the mapping function
  ...
  useEffect(() => {
    ...
    if (filterStatus !== "All" && filterPriority !== "All") {
      filtered = filtered.filter(
        (task) =>
          task.status === filterStatus && task.priority === filterPriority
      );
